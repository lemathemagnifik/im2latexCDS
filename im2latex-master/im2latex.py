...
def load_data():
  vocab = open('data/latex_vocab.txt','r',encoding='latin-1').read().split('\n')
  vocab_to_idx = dict([ (vocab[i],i) for i in range(len(vocab))])
  formulas = open('data/formulas.norm.lst','r',encoding='latin-1').read().split('\n')
...
def build_model(inp, batch_size, num_rows, num_columns, dec_seq_len):
  ...
  def fn(inp):
    ...
    with tf.variable_scope('encoder_rnn'):
      with tf.variable_scope('forward'):
        ...
        lstm_cell_fw = tf.contrib.rnn.LSTMCell(enc_lstm_dim)
        init_fw = tf.contrib.rnn.LSTMStateTuple(\
                  tf.get_variable("enc_fw_c", enc_init_shape),\
                  tf.get_variable("enc_fw_h", enc_init_shape)
                  )
      with tf.variable_scope('backward'):
        lstm_cell_bw = tf.contrib.rnn.LSTMCell(enc_lstm_dim)
        init_bw = tf.contrib.rnn.LSTMStateTuple(\
  ...
  dec_init_state = tf.contrib.rnn.LSTMStateTuple( tf.truncated_normal(dec_init_shape),\
